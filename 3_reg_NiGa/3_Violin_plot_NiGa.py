...
Energy_alloys_train = np.array(sort_list(data_pd.Er, len_data_train, index_sort))
Features_surf_alloys_train = sort_list(data_pd.Features_surf, len_data_train, index_sort)
...
ads_alloys_train = sort_list(data_pd.Reaction, len_data_train, index_sort)
unique_reaction = list(dict.fromkeys(ads_alloys_train))

xtics_reac = ["$C*+2O*$\n$+3H_{2(g)}$", "$CH_3O*+O*$\n"+r"$+\frac{3}{2}H_2{(g)}$", "$HCOO*$\n"+r"$+\frac{5}{2}H_{2(g)}$",
              "$CO*+O*$\n$+3H_{2(g)}$", "$C*+H_{2(g)}$\n$+2H_2O_{(g)}$", "$CO*+2H_{2(g)}$\n$+H_2O_{(g)}$",
              "$CH_3O*$\n"+r"$+H_2O_{(g)}+\frac{1}{2}H_2{(g)}$"]

index_ads = []
for ads in unique_reaction:
    index_ads.append(np.array(np.where(ads==np.array(ads_alloys_train))[0],dtype=int))
...
ax.set_title('Violin plot for reaction step')
ax.set_xlabel('Reaction step')
ax.set_ylabel('Reaction Energy [eV]')
ax.set_xticks(np.arange(1, len(unique_reaction)+1))
ax.set_xticklabels(xtics_reac, rotation=45, fontsize=8)

# Show plot
plt.tight_layout()
plt.savefig('violin_NiGa_ads_energy.png', dpi=600, bbox_inches='tight', pad_inches=0)
plt.show()
...
# Show plot
plt.tight_layout()
plt.savefig('violin_NiGa_mae_ads_energy.png', dpi=600, bbox_inches='tight', pad_inches=0)
plt.show()